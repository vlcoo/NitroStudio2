...
public class Bank : IOFile, PlayableBank
{
    ...
    /// </summary>
    public static double[] AttackTable =
    {
    ...
    /// </summary>
    public static double[] DecayReleaseTable =
    {
    ...
    /// </summary>
    public static double[] MaxReleaseTimes =
    {
        ...
        switch (i.Type())
        {
            case InstrumentType.DrumSet:
                n = new DrumSetInstrument
                    { Index = i.Index, NoteInfo = new List<NoteInfo>(), Min = ((DrumSetInstrument)i).Min };
                foreach (var r in i.NoteInfo) n.NoteInfo.Add(r.Duplicate());
                break;
            ...
            default:
                n = new DirectInstrument
                    { Index = i.Index, NoteInfo = new List<NoteInfo> { i.NoteInfo[0].Duplicate() } };
                break;
        }
    ...
    public override void Read(FileReader r)
    {
        ...
        //Read info.
        r.ReadUInt32s(8);
        var numInsts = r.ReadUInt32();
        var records = new List<InstrumentType>();
    ...
    public override void Write(FileWriter w)
    {
        ...
        if (Instruments.Count > 0)
        {
            ...
            var bakPos = new Dictionary<int, long>();
            for (var i = 0; i <= Instruments.Last().Index; i++)
            ...
            for (var i = 0; i < sortedInsts.Count; i++)
            {
                var bak = w.Position;
    ...
    {
        //New DLS.
        var d = new DownloadableSounds();

        //Wave map.
        var waveMap = new Dictionary<uint, RiffWave>();
        var psgMap = new Dictionary<ushort, RiffWave>();
        var noiseMap = new Dictionary<ushort, RiffWave>();
        ...
        {
            //New instrument.
            var im = new GotaSoundBank.DLS.Instrument();
            im.BankId = (uint)(inst.Index / 128);
            im.InstrumentId = (uint)(inst.Index % 128);
            im.Name = "Instrument " + im.InstrumentId;

            //Add regions.
            var lastNote = inst as DrumSetInstrument != null ? (inst as DrumSetInstrument).Min : (byte)0;
            foreach (var n in inst.NoteInfo)
            {
                ...
                switch (n.InstrumentType)
                {
                    ...
                    case InstrumentType.PSG:
                        ...
                        {
                            var psg = new RiffWave("Hardware/DutyCycle" + (n.WaveId + 1) + ".wav");
                    ...
                    case InstrumentType.Noise:
                        ...
                        {
                            var noise = new RiffWave("Hardware/WhiteNoise.wav");
    ...
    {
        var num = (value + 500) / 1000d;
        byte pan = 64;
        if (value != 0) pan = (byte)(num * 127);
        return pan;
    }
    ...
    public static byte GetNearestTableIndex(double value, double[] table)
    {
        ...
        {
            var dist = Math.Abs(value - table[i]);
            if (dist < minDis)
            {
                minDis = dist;
                ret = i;
