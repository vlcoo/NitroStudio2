...
public class Wave : SoundFile
{
    ...
    public void WriteShortened(FileWriter w)
    {
        ...
        var nTimeSampleRate = (ushort)(16756991 / SampleRate);
        if (BackupNTime != 0)
            w.Write(BackupNTime);
        else
        ...
        if (Loops)
            ...
            w.Write((uint)((Audio.DataSize - Sample2Offset((uint)(pcmFormat == PcmFormat.Encoded ? 1 : 0), pcmFormat)) /
                           4));
        Audio.Write(w);
    }
