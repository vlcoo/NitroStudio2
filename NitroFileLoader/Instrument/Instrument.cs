...
public abstract class Instrument : IReadable, IWriteable
{
    ...
    ///     Note info.
    /// </summary>
    public List<NoteInfo> NoteInfo = new();
    ...
    public NoteInfo GetNoteInfo(Notes note)
    {
        ...
        switch (Type())
        {
            ...
            case InstrumentType.KeySplit:
                ...
                //New algorithm.
                if ((Type() == InstrumentType.DrumSet && (byte)note < (this as DrumSetInstrument).Min) ||
                    (byte)note > NoteInfo.Select(x => (byte)x.Key).ElementAt(NoteInfo.Count - 1)) return null;
                for (var i = 0; i < NoteInfo.Count; i++)
