...
public class SoundArchive : IOFile
{
    ...
    ///     Banks.
    /// </summary>
    public List<BankInfo> Banks = new();
    ...
    ///     Sequence archives.
    /// </summary>
    public List<SequenceArchiveInfo> SequenceArchives = new();

    /// <summary>
    ///     Sequences.
    /// </summary>
    public List<SequenceInfo> Sequences = new();

    /// <summary>
    ///     Stream players.
    /// </summary>
    public List<StreamPlayerInfo> StreamPlayers = new();
    ...
    ///     Wave archives.
    /// </summary>
    public List<WaveArchiveInfo> WaveArchives = new();
    ...
    public override void Read(FileReader r)
    {
        ...
        var seqArcNames = new List<string>();
        var seqArcSequenceNames = new List<List<string>>();
        var bankNames = new List<string>();
        var warNames = new List<string>();
        var playerNames = new List<string>();
        var groupNames = new List<string>();
        var streamPlayerNames = new List<string>();
        ...
        //Read entries.
        var numFiles = r.ReadUInt32();
        var fileOffs = new List<Tuple<uint, uint>>();
        ...
        var md5Ids = new Dictionary<string, List<uint>>();
        ...
        foreach (var o in offs)
        {
            if (o != 0)
            {
                ...
                StreamPlayers.Last().Name =
                    ind > streamPlayerNames.Count - 1 ? "STRM_PLAYER_" + ind : streamPlayerNames[ind];
            }
        ...
        foreach (var o in offs)
        {
            if (o != 0)
            {
                ...
                var md5 = WaveArchives.Last().File.Md5Sum;
        ...
        foreach (var o in offs)
        {
            if (o != 0)
            {
                ...
                Banks.Last().WaveArchives[0] = Banks.Last().ReadingWave0Id == 0xFFFF
                    ? null
                    : WaveArchives.Where(x => x.Index == Banks.Last().ReadingWave0Id).FirstOrDefault();
                Banks.Last().WaveArchives[1] = Banks.Last().ReadingWave1Id == 0xFFFF
                    ? null
                    : WaveArchives.Where(x => x.Index == Banks.Last().ReadingWave1Id).FirstOrDefault();
                Banks.Last().WaveArchives[2] = Banks.Last().ReadingWave2Id == 0xFFFF
                    ? null
                    : WaveArchives.Where(x => x.Index == Banks.Last().ReadingWave2Id).FirstOrDefault();
                Banks.Last().WaveArchives[3] = Banks.Last().ReadingWave3Id == 0xFFFF
                    ? null
                    : WaveArchives.Where(x => x.Index == Banks.Last().ReadingWave3Id).FirstOrDefault();
                var md5 = Banks.Last().File.Md5Sum;
                if (!md5Ids.ContainsKey(md5))
                {
                ...
                {
                    if (!md5Ids[md5].Contains(Banks.Last().ReadingFileId)) Banks.Last().ForceIndividualFile = true;
                }
            }
        ...
        foreach (var o in offs)
        {
            if (o != 0)
            {
                ...
                Sequences.Last().Player =
                    Players.Where(x => x.Index == Sequences.Last().ReadingPlayerId).FirstOrDefault();
                var md5 = Sequences.Last().File.Md5Sum;
                if (!md5Ids.ContainsKey(md5))
                {
        ...
        foreach (var o in offs)
        {
            if (o != 0)
            {
                ...
                Streams.Last().Player =
                    StreamPlayers.Where(x => x.Index == Streams.Last().ReadingPlayerId).FirstOrDefault();
                var md5 = Streams.Last().File.Md5Sum;
                if (!md5Ids.ContainsKey(md5))
                {
                ...
                {
                    if (!md5Ids[md5].Contains(Streams.Last().ReadingFileId)) Streams.Last().ForceIndividualFile = true;
                }
            }
        ...
        foreach (var o in offs)
        {
            if (o != 0)
            {
                ...
                {
                    var seqNum = 0;
                    for (var i = 0; i <= SequenceArchives.Last().File.Sequences.Last().Index; i++)
                    {
                        var defName = "Sequence_" + i;
                        try
                        {
                ...
                var md5 = SequenceArchives.Last().File.Md5Sum;
                if (!md5Ids.ContainsKey(md5))
                {
        ...
        foreach (var o in offs)
        {
            if (o != 0)
            {
                ...
                for (var i = 0; i < Groups.Last().Entries.Count; i++)
                    switch (Groups.Last().Entries[i].Type)
                    {
                        case GroupEntryType.Sequence:
                            Groups.Last().Entries[i].Entry = Sequences
                                .Where(x => x.Index == (int)Groups.Last().Entries[i].ReadingId).FirstOrDefault();
                            break;
    ...
    public override void Write(FileWriter w)
    {
        ...
        if (SaveSymbols)
        {
            ...
            if (SequenceArchives.Count > 1)
            {
                ...
                for (var i = 0; i <= SequenceArchives.Last().Index; i++)
                {
            ...
            //Sub entries.
            var seqArcSeqSBak = new List<long>();
            if (SequenceArchives.Count > 1)
                for (var i = 0; i <= SequenceArchives.Last().Index; i++)
                {
            ...
            void writeStringData(object entryList, long tablePos)
            {
                ...
                if (seqList != null)
                    for (var i = 0; i <= seqList.Last().Index; i++)
                    {
                ...
                if (bnkList != null)
                    for (var i = 0; i <= bnkList.Last().Index; i++)
                    {
                ...
                if (warList != null)
                    for (var i = 0; i <= warList.Last().Index; i++)
                    {
                ...
                if (plyList != null)
                    for (var i = 0; i <= plyList.Last().Index; i++)
                    {
                ...
                if (grpList != null)
                    for (var i = 0; i <= grpList.Last().Index; i++)
                    {
                ...
                if (stmPlyList != null)
                    for (var i = 0; i <= stmPlyList.Last().Index; i++)
                    {
                ...
                if (stmList != null)
                    for (var i = 0; i <= stmList.Last().Index; i++)
                    {
            ...
            if (SequenceArchives.Count > 0)
                for (var i = 0; i <= SequenceArchives.Last().Index; i++)
                {
                    ...
                    if (e != null)
                    {
                        ...
                        //Write names.
                        if (e.File.Sequences.Count > 0)
                            for (var j = 0; j <= e.File.Sequences.Last().Index; j++)
                            {
                                var f = e.File.Sequences.Where(x => x.Index == j).FirstOrDefault();
                                if (f == null) continue;
                                var currPos = w.Position;
            ...
            //Close block.
            var beforePadPosS = w.Position;
            w.Pad(4);
            var afterPadPosS = w.Position;
            w.CloseBlock();
            w.BlockSizes[w.BlockSizes.Count - 1] -= afterPadPosS - beforePadPosS;
        }

        //Get files.
        var files = new Dictionary<string, Tuple<IOFile, uint>>();
        uint fileId = 0;
        foreach (var e in Sequences)
        {
            var md5 = e.File.Md5Sum;
        ...
        //Close info block.
        var beforePadPosI = w.Position;
        w.Pad(4);
        var afterPadPosI = w.Position;
        w.CloseBlock();
        ...
        //Get binaries.
        var filesRaw = new List<byte[]>();
        foreach (var f in files) filesRaw.Add(f.Value.Item1.Write());

        //Init offsets.
        w.Write((uint)files.Count);
        for (var i = 0; i < filesRaw.Count; i++)
        {
            w.InitOffset("file" + i);
        ...
        //Close block.
        var beforePadPosFAT = w.Position;
        w.Pad(4);
        var afterPadPosFAT = w.Position;
        w.CloseBlock();
    ...
    {
        //SBDL first.
        var sbdl = new List<string>();
        foreach (var e in Players) sbdl.Add("#define " + e.Name + "\t" + e.Index);
        foreach (var e in WaveArchives) sbdl.Add("#define " + e.Name + "\t" + e.Index);
        foreach (var e in StreamPlayers) sbdl.Add("#define " + e.Name + "\t" + e.Index);
        foreach (var e in Streams) sbdl.Add("#define " + e.Name + "\t" + e.Index);
        foreach (var e in Banks) sbdl.Add("#define " + e.Name + "\t" + e.Index);
        foreach (var e in Sequences) sbdl.Add("#define " + e.Name + "\t" + e.Index);
        foreach (var e in SequenceArchives) sbdl.Add("#define " + e.Name + "\t" + e.Index);
        foreach (var e in Groups) sbdl.Add("#define " + e.Name + "\t" + e.Index);
        File.WriteAllLines(directory + "/" + projectName + ".sbdl", sbdl);

        //SPRJ.
        var sprj = new List<string>();
        ...
        //Wave files.
        var waveFiles = new Dictionary<int, string>();
        var waveMd5sums = new Dictionary<string, string>();
        foreach (var e in WaveArchives)
        {
            //MD5SUM.
            var md5 = e.File.Md5Sum;
        ...
        //Stream files.
        var strmFiles = new Dictionary<int, string>();
        var strmMd5sums = new Dictionary<string, string>();
        foreach (var e in Streams)
        {
            //MD5SUM.
            var md5 = e.File.Md5Sum;
        ...
        //Bank files.
        var bnkFiles = new Dictionary<int, string>();
        var bnkMd5sums = new Dictionary<string, string>();
        foreach (var e in Banks)
        {
            //MD5SUM.
            var md5 = e.File.Md5Sum;
        ...
        //Sequence files.
        var seqFiles = new Dictionary<int, string>();
        var seqMd5sums = new Dictionary<string, string>();
        foreach (var e in Sequences)
        {
            //MD5SUM.
            var md5 = e.File.Md5Sum;
        ...
        var seqArcFiles = new Dictionary<int, string>();
        var seqArcMd5sums = new Dictionary<string, string>();
        foreach (var e in SequenceArchives)
        {
            //MD5SUM.
            var md5 = e.File.Md5Sum;
        ...
        foreach (var e in Players)
        {
            var bitFlags = e.BitFlags();
            if (bitFlags == 0xFFFF) bitFlags = 0;
            var index = "";
            if (id != e.Index)
            {
                id = e.Index;
        ...
        foreach (var e in WaveArchives)
        {
            ...
            id++;
            sarc.Add(e.Name + index + "\t: TEXT, \"" + waveFiles[e.Index] + ".swls\"" +
        ...
        foreach (var e in Banks)
        {
            ...
            //Stuff.
            var stuff = e.Name + index + "\t: " + (text ? "TEXT" : "BIN") + ", \"" + bnkFiles[e.Index] + "." +
                        (text ? "" : "s") + "bnk\"" + ", ";

            //Wave archive names.
            var wars = new string[4];
            for (var i = 0; i < wars.Length; i++)
                if (e.WaveArchives[i] == null)
                    switch (i)
                    {
                        case 0:
                            if (e.ReadingWave0Id != 0xFFFF) wars[i] = e.ReadingWave0Id.ToString();
                            break;
                        case 1:
                            if (e.ReadingWave1Id != 0xFFFF) wars[i] = e.ReadingWave1Id.ToString();
                            break;
                        case 2:
                            if (e.ReadingWave2Id != 0xFFFF) wars[i] = e.ReadingWave2Id.ToString();
                            break;
                        case 3:
                            if (e.ReadingWave3Id != 0xFFFF) wars[i] = e.ReadingWave3Id.ToString();
                            break;
                    }
                else
                    wars[i] = e.WaveArchives[i].Name;

            if (wars[0] != null) stuff += wars[0];
            if (wars[1] != null || wars[2] != null || wars[3] != null) stuff += ", ";

            if (wars[1] != null) stuff += wars[1];
            if (wars[2] != null || wars[3] != null) stuff += ", ";

            if (wars[2] != null) stuff += wars[2];
            if (wars[3] != null) stuff += ", ";

            if (wars[3] != null) stuff += wars[3];
        ...
        foreach (var e in Sequences)
        {
            ...
            id++;
            sarc.Add(e.Name + index + "\t: TEXT, \"" + seqFiles[e.Index] + ".smft\", " +
                     (e.Bank == null ? e.ReadingBankId.ToString() : e.Bank.Name) + ", " + e.Volume + ", " +
                     e.ChannelPriority + ", " + e.PlayerPriority + ", " +
        ...
        foreach (var e in StreamPlayers)
        {
            ...
            id++;
            sarc.Add(e.Name + index + "\t: " + (e.IsStereo ? "STEREO" : "MONO") + ", " + e.LeftChannel +
        ...
        foreach (var e in Streams)
        {
            ...
            id++;
            sarc.Add(e.Name + index + "\t: " + "STRM" + ", \"" + strmFiles[e.Index] + ".strm\", " + e.Volume + ", " +
                     e.Priority + ", " + (e.Player == null ? e.ReadingPlayerId.ToString() : e.Player.Name));
        }
