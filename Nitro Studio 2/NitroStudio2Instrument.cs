...
public class NitroStudio2Instrument : IOFile
{
    ...
    public NitroStudio2Instrument(Instrument inst, SoundArchive s, ushort war0, ushort war1, ushort war2, ushort war3)
    {
        ...
        foreach (var n in inst.NoteInfo)
        {
            ...
            if (n.WarId != 0xFFFF)
            {
                var war = s.WaveArchives.Where(x => x.Index == n.WarId).FirstOrDefault();
                if (war != null) w.Wave = war.File.Waves[n.WaveId];
            }
    ...
    public override void Read(FileReader r)
    {
        ...
        //Read data.
        Inst.Read(r);
        if (Inst as DirectInstrument != null) Inst.NoteInfo[0].InstrumentType = (InstrumentType)r.ReadByte();

        //Read waves.
        Waves = null;
        if (!r.ReadBoolean()) return;
        Waves = new List<WaveEntry>();
        var numWaves = r.ReadUInt32();
        for (uint i = 0; i < numWaves; i++)
        {
            ...
            if (r.ReadBoolean())
            {
                ...
                Waves[Waves.Count - 1].Wave = r.ReadFile<Wave>();
            }
        }
    }
    ...
    /// <param name="war2">Wave archive 2.</param>
    /// <param name="war3">Wave archive 3.</param>
    public void WriteInstrument(Bank bnk, int instrumentId, SoundArchive a, ushort war0, ushort war1, ushort war2,
        ushort war3)
    {
        //Set the instrument.
        ...
        //Get wave archives.
        var wars = new WaveArchiveInfo[4];
        if (war0 != 0xFFFF) wars[0] = a.WaveArchives.Where(x => x.Index == war0).FirstOrDefault();
        if (war1 != 0xFFFF) wars[1] = a.WaveArchives.Where(x => x.Index == war1).FirstOrDefault();
        if (war2 != 0xFFFF) wars[2] = a.WaveArchives.Where(x => x.Index == war2).FirstOrDefault();
        if (war3 != 0xFFFF) wars[3] = a.WaveArchives.Where(x => x.Index == war3).FirstOrDefault();

        //Make sure there are linked wave archives.
        if (wars.Where(x => x != null).Count() < 1) return;

        //For each region in the instrument.
        foreach (var r in Inst.NoteInfo)
        {
            //PCM type.
            if (r.InstrumentType != InstrumentType.PCM) continue;
            ...
            //Wave is not null.
            if (e.Wave == null) continue;

            //Get MD5SUM of wave.
            var md5 = e.Wave.Md5Sum;

            //Try and find matching wave.
            var found = false;
            for (var i = 0; i < wars.Length; i++)
                if (wars[i] != null)
                    for (var j = 0; j < wars[i].File.Waves.Count; j++)
            ...
            if (!found)
            {
                ...
                r.WaveId = (ushort)(a.WaveArchives.Where(x => x.Index == mapper.WarMap[0]).FirstOrDefault().File.Waves
                    .Count() - 1);
                r.WarId = (ushort)wars.ToList()
                    .IndexOf(a.WaveArchives.Where(x => x.Index == mapper.WarMap[0]).FirstOrDefault());
            }
        }
    ...
    public override void Write(FileWriter w)
    {
        ...
        //Write instrument.
        w.Write(Inst);
        if (Inst as DirectInstrument != null) w.Write((byte)Inst.NoteInfo[0].InstrumentType);
        ...
        foreach (var v in Waves)
        {
            ...
            w.Write(v.WarId);
            w.Write(v.Wave != null);
            if (v.Wave != null) w.WriteFile(v.Wave);
        }
    }
