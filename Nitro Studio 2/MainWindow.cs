...
using Instrument = GotaSoundBank.DLS.Instrument;
...
public class MainWindow : EditorBase
{
    ...
    public MainWindow(string fileToOpen) : base(typeof(SoundArchive), "Sound Archive", "dat", "Nitro Studio 2",
        fileToOpen, null)
    {
        Init();
    }
    ...
    public void Init()
    {
        //Window stuff.
        Icon = Resources.Icon;
        FormClosing += SAClosing;
        toolsToolStripMenuItem.Visible = true;

        //Settings.
        writeNamesBox.CheckedChanged += WriteNamesChanged;
        seqImportModeBox.SelectedIndex = 0;
        seqExportModeBox.SelectedIndex = 0;

        //Index panel.
        swapAtIndexButton.Click += SwapAtIndexButtonPressed;

        //Unique Id panel.
        forceUniqueFileBox.CheckedChanged += ForceUniqueIdChanged;

        //Sequence stuff.
        seqVolumeBox.ValueChanged += SequenceVolumeChanged;
        seqChannelPriorityBox.ValueChanged += SequenceChannelPriorityChanged;
        seqPlayerPriorityBox.ValueChanged += SequencePlayerPriorityChanged;
        seqBankBox.ValueChanged += SequenceBankBoxChanged;
        seqBankComboBox.SelectedIndexChanged += SequenceBankComboBoxChanged;
        seqPlayerBox.ValueChanged += SequencePlayerBoxChanged;
        seqPlayerComboBox.SelectedIndexChanged += SequencePlayerComboBoxChanged;

        //Sequence archive stuff.
        seqArcOpenFileButton.Click += OpenSeqArcFile;

        //Bank stuff.
        bnkWar0Box.ValueChanged += BnkWar0BoxChanged;
        bnkWar1Box.ValueChanged += BnkWar1BoxChanged;
        bnkWar2Box.ValueChanged += BnkWar2BoxChanged;
        bnkWar3Box.ValueChanged += BnkWar3BoxChanged;
        bnkWar0ComboBox.SelectedValueChanged += BnkWar0ComboBoxChanged;
        bnkWar1ComboBox.SelectedValueChanged += BnkWar1ComboBoxChanged;
        bnkWar2ComboBox.SelectedValueChanged += BnkWar2ComboBoxChanged;
        bnkWar3ComboBox.SelectedValueChanged += BnkWar3ComboBoxChanged;

        //Wave archive stuff.
        loadIndividuallyBox.CheckedChanged += WarLoadIndividualChanged;

        //Player stuff.
        playerMaxSequencesBox.ValueChanged += PlayerSequenceMaxChanged;
        playerHeapSizeBox.ValueChanged += PlayerHeapSizeChanged;
        playerFlag0Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag1Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag2Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag3Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag4Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag5Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag6Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag7Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag8Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag9Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag10Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag11Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag12Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag13Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag14Box.CheckedChanged += PlayerFlagsChanged;
        playerFlag15Box.CheckedChanged += PlayerFlagsChanged;

        //Group stuff.
        grpEntries.CellValueChanged += GroupEntriesChanged;
        grpEntries.RowsRemoved += GroupEntriesChanged;

        //Stream player stuff.
        stmPlayerChannelType.SelectedIndexChanged += StreamPlayerTypeChanged;
        stmPlayerLeftChannelBox.ValueChanged += StreamPlayerLeftChannelChanged;
        stmPlayerRightChannelBox.ValueChanged += StreamPlayerRightChannelChanged;

        //Stream stuff.
        stmVolumeBox.ValueChanged += StreamVolumeChanged;
        stmPriorityBox.ValueChanged += StreamPriorityChanged;
        stmPlayerBox.ValueChanged += StreamPlayerBoxChanged;
        stmPlayerComboBox.SelectedIndexChanged += StreamPlayerComboBoxChanged;
        stmMonoToStereoBox.CheckedChanged += StreamMonoToStereoChanged;

        //Player.
        Player = new Player(Mixer);
        kermalisPlayButton.Click += PlayClick;
        kermalisPauseButton.Click += PauseClick;
        kermalisStopButton.Click += StopClick;
        kermalisVolumeSlider.ValueChanged += VolumeChanged;
        kermalisLoopBox.CheckedChanged += LoopChanged;
        kermalisPosition.MouseUp += PositionMouseUp;
        kermalisPosition.MouseDown += PositionMouseDown;
        tree.KeyPress += KeyPress;
    ...
    public override void UpdateNodes()
    {
        ...
        {
            //Root menus.
            for (var i = 1; i < 9; i++) tree.Nodes[i].ContextMenuStrip = rootMenu;

            //Load data.
            foreach (var e in SA.Sequences)
            {
                tree.Nodes["sequences"].Nodes.Add("entry" + e.Index, "[" + e.Index + "] " + e.Name, 2, 2);
                tree.Nodes["sequences"].Nodes["entry" + e.Index].ContextMenuStrip = CreateMenuStrip(sarEntryMenu,
                    new[] { 0, 1, 4, 5, 6, 7 },
                    new EventHandler[] { AddAbove, AddBelow, Replace, Export, Rename, Delete });
            }

            foreach (var e in SA.SequenceArchives)
            {
                tree.Nodes["sequenceArchives"].Nodes.Add("entry" + e.Index, "[" + e.Index + "] " + e.Name, 3, 3);
                tree.Nodes["sequenceArchives"].Nodes["entry" + e.Index].ContextMenuStrip = CreateMenuStrip(sarEntryMenu,
                    new[] { 0, 1, 4, 5, 6, 7 },
                    new EventHandler[] { AddAbove, AddBelow, Replace, Export, Rename, Delete });
                foreach (var s in e.File.Sequences)
                {
                    tree.Nodes["sequenceArchives"].Nodes["entry" + e.Index].Nodes
                        .Add("entry" + s.Index, "[" + s.Index + "] " + s.Name, 2, 2);
                    tree.Nodes["sequenceArchives"].Nodes["entry" + e.Index].Nodes["entry" + s.Index].ContextMenuStrip =
                        CreateMenuStrip(sarEntryMenu, new[] { 5, 6 }, new EventHandler[] { Export, Rename });
                }
            }
            ...
            foreach (var e in SA.Banks)
            {
                ...
                tree.Nodes["banks"].Nodes["entry" + e.Index].ContextMenuStrip = CreateMenuStrip(sarEntryMenu,
                    new[] { 0, 1, 4, 5, 6, 7 },
                    new EventHandler[] { AddAbove, AddBelow, Replace, Export, Rename, Delete });
            }
            ...
            foreach (var e in SA.WaveArchives)
            {
                ...
                tree.Nodes["waveArchives"].Nodes["entry" + e.Index].ContextMenuStrip = CreateMenuStrip(sarEntryMenu,
                    new[] { 0, 1, 4, 5, 6, 7 },
                    new EventHandler[] { AddAbove, AddBelow, Replace, Export, Rename, Delete });
            }
            ...
            foreach (var e in SA.Players)
            {
                ...
                tree.Nodes["players"].Nodes["entry" + e.Index].ContextMenuStrip = CreateMenuStrip(sarEntryMenu,
                    new[] { 0, 1, 6, 7 }, new EventHandler[] { AddAbove, AddBelow, Rename, Delete });
            }

            foreach (var e in SA.Groups)
            {
                tree.Nodes["groups"].Nodes.Add("entry" + e.Index, "[" + e.Index + "] " + e.Name, 7, 7);
                tree.Nodes["groups"].Nodes["entry" + e.Index].ContextMenuStrip = CreateMenuStrip(sarEntryMenu,
                    new[] { 0, 1, 6, 7 }, new EventHandler[] { AddAbove, AddBelow, Rename, Delete });
            }
            ...
            foreach (var e in SA.StreamPlayers)
            {
                ...
                tree.Nodes["streamPlayers"].Nodes["entry" + e.Index].ContextMenuStrip = CreateMenuStrip(sarEntryMenu,
                    new[] { 0, 1, 6, 7 }, new EventHandler[] { AddAbove, AddBelow, Rename, Delete });
            }
            ...
            foreach (var e in SA.Streams)
            {
                ...
                tree.Nodes["streams"].Nodes["entry" + e.Index].ContextMenuStrip = CreateMenuStrip(sarEntryMenu,
                    new[] { 0, 1, 4, 5, 6, 7 },
                    new EventHandler[] { AddAbove, AddBelow, Replace, Export, Rename, Delete });
            }
        }
        else
        {
            ...
            foreach (TreeNode n in tree.Nodes) n.ContextMenuStrip = null;
        }
        ...
        else
        {
            ...
            if (tree.SelectedNode.Parent.Parent == null)
            {
                ...
                else if (tree.SelectedNode.Parent.Name == "sequenceArchives")
                {
                    ...
                    forceUniqueFilePanel.Show();
                    seqArcPanel.Show();
                    var e = SA.SequenceArchives.Where(x => x.Index == GetIdFromNode(tree.SelectedNode))
                        .FirstOrDefault();
                    itemIndexBox.Value = e.Index;
                    forceUniqueFileBox.Checked = e.ForceIndividualFile;
                    status.Text = "[" + e.Index + "] " + e.Name + " Selected. File Is " + GetBytesSize(e.File) + ".";
                ...
                else if (tree.SelectedNode.Parent.Name == "banks")
                {
                    ...
                    SetWaveArchiveIndex(SA, bnkWar0ComboBox,
                        e.WaveArchives[0] == null ? e.ReadingWave0Id : (ushort)e.WaveArchives[0].Index);
                    SetWaveArchiveIndex(SA, bnkWar1ComboBox,
                        e.WaveArchives[1] == null ? e.ReadingWave1Id : (ushort)e.WaveArchives[1].Index);
                    SetWaveArchiveIndex(SA, bnkWar2ComboBox,
                        e.WaveArchives[2] == null ? e.ReadingWave2Id : (ushort)e.WaveArchives[2].Index);
                    SetWaveArchiveIndex(SA, bnkWar3ComboBox,
                        e.WaveArchives[3] == null ? e.ReadingWave3Id : (ushort)e.WaveArchives[3].Index);
                    SetWaveArchiveIndex(SA, bnkWar0Box,
                        e.WaveArchives[0] == null ? e.ReadingWave0Id : (ushort)e.WaveArchives[0].Index);
                    SetWaveArchiveIndex(SA, bnkWar1Box,
                        e.WaveArchives[1] == null ? e.ReadingWave1Id : (ushort)e.WaveArchives[1].Index);
                    SetWaveArchiveIndex(SA, bnkWar2Box,
                        e.WaveArchives[2] == null ? e.ReadingWave2Id : (ushort)e.WaveArchives[2].Index);
                    SetWaveArchiveIndex(SA, bnkWar3Box,
                        e.WaveArchives[3] == null ? e.ReadingWave3Id : (ushort)e.WaveArchives[3].Index);
                    status.Text = "[" + e.Index + "] " + e.Name + " Selected. File Is " + GetBytesSize(e.File) + ".";
                }
                ...
                else if (tree.SelectedNode.Parent.Name == "streams")
                {
                    ...
                    SetStreamPlayerIndex(SA, stmPlayerComboBox,
            ...
            else
            {
                ...
                kermalisSoundPlayerPanel.Show();
                blankPanel.Show();
                var e = SA.SequenceArchives.Where(x => x.Index == GetIdFromNode(tree.SelectedNode.Parent))
                    .FirstOrDefault().File.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode))
                    .FirstOrDefault();
                status.Text = "[" + e.Index + "] " + e.Name + " Selected.";
            }
        }
    ...
    public override void NodeMouseDoubleClick()
    {
        ...
        if (tree.SelectedNode.Parent != null)
        {
            ...
            {
                var e = SA.Banks.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault();
                var ed = new BankEditor(e.File, this, e.Name);
                SetWaveArchiveIndex(SA, ed.war0ComboBox,
                    e.WaveArchives[0] == null ? e.ReadingWave0Id : (ushort)e.WaveArchives[0].Index);
                SetWaveArchiveIndex(SA, ed.war1ComboBox,
                    e.WaveArchives[1] == null ? e.ReadingWave1Id : (ushort)e.WaveArchives[1].Index);
                SetWaveArchiveIndex(SA, ed.war2ComboBox,
                    e.WaveArchives[2] == null ? e.ReadingWave2Id : (ushort)e.WaveArchives[2].Index);
                SetWaveArchiveIndex(SA, ed.war3ComboBox,
                    e.WaveArchives[3] == null ? e.ReadingWave3Id : (ushort)e.WaveArchives[3].Index);
                SetWaveArchiveIndex(SA, ed.war0Box,
                    e.WaveArchives[0] == null ? e.ReadingWave0Id : (ushort)e.WaveArchives[0].Index);
                SetWaveArchiveIndex(SA, ed.war1Box,
                    e.WaveArchives[1] == null ? e.ReadingWave1Id : (ushort)e.WaveArchives[1].Index);
                SetWaveArchiveIndex(SA, ed.war2Box,
                    e.WaveArchives[2] == null ? e.ReadingWave2Id : (ushort)e.WaveArchives[2].Index);
                SetWaveArchiveIndex(SA, ed.war3Box,
                    e.WaveArchives[3] == null ? e.ReadingWave3Id : (ushort)e.WaveArchives[3].Index);
                ed.LoadWaveArchives();
                ed.Show();
            }
            ...
            {
                var e = SA.WaveArchives.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault();
                var ed = new WaveArchiveEditor(e.File, this, e.Name);
                ed.Show();
            }
            ...
            else if (tree.SelectedNode.Parent == tree.Nodes["streams"])
            {
                ...
                var p = new StreamPlayer(this, NitroPath + "/" + "tmpStream" + (StreamTempCount - 1) + ".wav", s.Name);
                p.Show();
            }
        }
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo) SA.SaveSymbols = writeNamesBox.Checked;
    }
    ...
    {
        //Get index.
        var index = (int)itemIndexBox.Value;
        var bakIndex = GetIdFromNode(tree.SelectedNode);
        ...
        switch (tree.SelectedNode.Parent.Name)
        {
            //Sequences.
            case "sequences":
                if ((uint)index > SoundArchive.MaxSequenceId) MessageBox.Show("Index is outside the max possible Id!");
                ...
                if (prevSeq != null) prevSeq.Index = bakIndex;
            ...
            case "sequenceArchives":
                ...
                if (prevSeqArc != null) prevSeqArc.Index = bakIndex;
            ...
            //Banks.
            case "banks":
                if ((uint)index > SoundArchive.MaxBankId) MessageBox.Show("Index is outside the max possible Id!");
                var prevBnk = SA.Banks.Where(x => x.Index == index).FirstOrDefault();
                SA.Banks.Where(x => x.Index == bakIndex).FirstOrDefault().Index = index;
                if (prevBnk != null) prevBnk.Index = bakIndex;
            ...
            case "waveArchives":
                ...
                if (prevWar != null) prevWar.Index = bakIndex;
            ...
            //Players.
            case "players":
                if ((uint)index > SoundArchive.MaxPlayerId) MessageBox.Show("Index is outside the max possible Id!");
                ...
                if (prevPly != null) prevPly.Index = bakIndex;
            ...
            //Groups.
            case "groups":
                if ((uint)index > SoundArchive.MaxGroupId) MessageBox.Show("Index is outside the max possible Id!");
                var prevGrp = SA.Groups.Where(x => x.Index == index).FirstOrDefault();
                SA.Groups.Where(x => x.Index == bakIndex).FirstOrDefault().Index = index;
                if (prevGrp != null) prevGrp.Index = bakIndex;
            ...
            case "streamPlayers":
                ...
                if (prevStmPly != null) prevStmPly.Index = bakIndex;
            ...
            //Streams.
            case "streams":
                if ((uint)index > SoundArchive.MaxStreamId) MessageBox.Show("Index is outside the max possible Id!");
                ...
                if (prevStm != null) prevStm.Index = bakIndex;
    ...
    public void ForceUniqueIdChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
            ...
            {
                case "sequences":
                    SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault()
                        .ForceIndividualFile = forceUniqueFileBox.Checked;
                    break;
                case "sequenceArchives":
                    SA.SequenceArchives.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault()
                        .ForceIndividualFile = forceUniqueFileBox.Checked;
                    break;
                case "banks":
                    SA.Banks.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault()
                        .ForceIndividualFile = forceUniqueFileBox.Checked;
                    break;
                case "waveArchives":
                    SA.WaveArchives.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault()
                        .ForceIndividualFile = forceUniqueFileBox.Checked;
                    break;
                case "streams":
                    SA.Streams.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault()
                        .ForceIndividualFile = forceUniqueFileBox.Checked;
                    break;
            }
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.WaveArchives.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().LoadIndividually =
                loadIndividuallyBox.Checked;
    }
    ...
    public static void PopulateWaveArchiveBox(SoundArchive a, ComboBox c)
    {
        ...
        foreach (var w in a.WaveArchives) c.Items.Add("[" + w.Index + "] - " + w.Name);
    }
    ...
    public void BnkWar0BoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            SA.Banks.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ReadingWave0Id =
    ...
    public void BnkWar1BoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            SA.Banks.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ReadingWave1Id =
    ...
    public void BnkWar2BoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            SA.Banks.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ReadingWave2Id =
    ...
    public void BnkWar3BoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            SA.Banks.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ReadingWave3Id =
    ...
    public void BnkWar0ComboBoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            var val = (ushort)bnkWar0ComboBox.SelectedIndex;
            if (val == 0)
                val = 0xFFFF;
            else if (val == 1)
                return;
    ...
    public void BnkWar1ComboBoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            var val = (ushort)bnkWar1ComboBox.SelectedIndex;
            if (val == 0)
                val = 0xFFFF;
            else if (val == 1)
                return;
    ...
    public void BnkWar2ComboBoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            var val = (ushort)bnkWar2ComboBox.SelectedIndex;
            if (val == 0)
                val = 0xFFFF;
            else if (val == 1)
                return;
    ...
    public void BnkWar3ComboBoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            var val = (ushort)bnkWar3ComboBox.SelectedIndex;
            if (val == 0)
                val = 0xFFFF;
            else if (val == 1)
                return;
    ...
    public void PopulateGroupGrid(DataGridView v, GroupInfo g)
    {
        ...
        c.Items.Clear();
        foreach (var e in SA.Sequences) c.Items.Add("[" + e.Index + "] " + e.Name + " (Sequence)");
        foreach (var e in SA.SequenceArchives) c.Items.Add("[" + e.Index + "] " + e.Name + " (Sequence Archive)");
        foreach (var e in SA.Banks) c.Items.Add("[" + e.Index + "] " + e.Name + " (Bank)");
        foreach (var e in SA.WaveArchives) c.Items.Add("[" + e.Index + "] " + e.Name + " (Wave Archive)");
        ...
        foreach (var e in g.Entries)
        {
            ...
            switch (e.Type)
            {
                case GroupEntryType.Sequence:
                    ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[0]).Value = "[" +
                        (e.Entry as SequenceInfo).Index + "] " + (e.Entry as SequenceInfo).Name + " (Sequence)";
                    ...
                    ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[1]).Items.Add(
                        "Sequence + Bank + Wave Archive");
                    if (e.LoadSequence && e.LoadBank && e.LoadWaveArchive)
                        ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[1]).Value =
                            "Sequence + Bank + Wave Archive";
                ...
                case GroupEntryType.SequenceArchive:
                    ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[0]).Value = "[" +
                        (e.Entry as SequenceArchiveInfo).Index + "] " + (e.Entry as SequenceArchiveInfo).Name +
                        " (Sequence Archive)";
                ...
                case GroupEntryType.Bank:
                    ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[0]).Value = "[" +
                        (e.Entry as BankInfo).Index + "] " + (e.Entry as BankInfo).Name + " (Bank)";
                    ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[1]).Items.Add("Bank");
                    ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[1]).Items.Add("Wave Archive");
                    ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[1]).Items.Add("Bank + Wave Archive");
                ...
                case GroupEntryType.WaveArchive:
                    ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[0]).Value = "[" +
                        (e.Entry as WaveArchiveInfo).Index + "] " + (e.Entry as WaveArchiveInfo).Name +
                        " (Wave Archive)";
                    ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[1]).Items.Add("Wave Archive");
                    ((DataGridViewComboBoxCell)v.Rows[v.Rows.Count - 2].Cells[1]).Value = "Wave Archive";
                    break;
    ...
    public void GroupEntriesChanged(object sender, EventArgs e)
    {
        ...
        if (FileOpen && File != null && !WritingInfo)
        {
            ...
            //For each row.
            for (var i = 1; i < grpEntries.Rows.Count; i++)
            {
                //Get the cells.
                ...
                var t = GroupEntryType.WaveArchive;
                object entry = null;
                uint readingId = 0;
                var loadWar = false;
                var loadBnk = false;
                var loadSeqArc = false;
                var loadSeq = false;
                var bakFlags = "";
                try
                {
                ...
                while (flagsCell.Items.Count > 1) flagsCell.Items.RemoveAt(flagsCell.Items.Count - 1);
                switch (((string)itemCell.Value).Split('(')[1].Split(')')[0])
                {
                    case "Sequence":
                        t = GroupEntryType.Sequence;
                        entry = SA.Sequences
                            .Where(x => x.Index == int.Parse(((string)itemCell.Value).Split('[')[1].Split(']')[0]))
                            .FirstOrDefault();
                ...
                {
                    Type = t, Entry = entry, ReadingId = readingId, LoadSequence = loadSeq,
                    LoadSequenceArchive = loadSeqArc, LoadBank = loadBnk, LoadWaveArchive = loadWar
                });
            }
    ...
    public void StreamPlayerTypeChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            ...
            else
            {
                ...
                rightChannelLabel.Enabled = true;
                stmPlayerRightChannelBox.Enabled = true;
                if (SA.StreamPlayers.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault()
                        .LeftChannel != 15)
                    SA.StreamPlayers.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault()
                        .RightChannel = (byte)(SA.StreamPlayers.Where(x => x.Index == GetIdFromNode(tree.SelectedNode))
                        .FirstOrDefault().LeftChannel + 1);
                stmPlayerRightChannelBox.Value = SA.StreamPlayers
                    .Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().RightChannel;
            }

            WritingInfo = false;
            SA.StreamPlayers.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().IsStereo =
                stmPlayerChannelType.SelectedIndex == 1;
        }
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.StreamPlayers.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().LeftChannel =
                (byte)stmPlayerLeftChannelBox.Value;
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.StreamPlayers.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().RightChannel =
                (byte)stmPlayerRightChannelBox.Value;
    }
    ...
    public static void PopulateStreamPlayerBox(SoundArchive a, ComboBox c)
    {
        c.Items.Clear();
        c.Items.Add("Other Index");
        foreach (var w in a.StreamPlayers) c.Items.Add("[" + w.Index + "] - " + w.Name);
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.Streams.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().Volume =
                (byte)stmVolumeBox.Value;
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.Streams.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().Priority =
                (byte)stmPriorityBox.Value;
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.Streams.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().MonoToStereo =
                stmMonoToStereoBox.Checked;
    }
    ...
    public void StreamPlayerComboBoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
            ...
            {
                WritingInfo = true;
                var index = byte.Parse(((string)stmPlayerComboBox.SelectedItem).Split('[')[1].Split(']')[0]);
                stmPlayerBox.Value = index;
                SA.Streams.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ReadingPlayerId =
                    index;
                SA.Streams.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().Player =
                    SA.StreamPlayers.Where(x => x.Index == index).FirstOrDefault();
                WritingInfo = false;
            }
    }
    ...
    public void StreamPlayerBoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            ...
            SA.Streams.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().Player =
                SA.StreamPlayers.Where(x => x.Index == (byte)stmPlayerBox.Value).FirstOrDefault();
            SA.Streams.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ReadingPlayerId =
                (byte)stmPlayerBox.Value;
            WritingInfo = false;
        }
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.Players.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().SequenceMax =
                (ushort)playerMaxSequencesBox.Value;
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.Players.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().HeapSize =
                (uint)playerHeapSizeBox.Value;
    }
    ...
    public static void PopulateBankBox(SoundArchive a, ComboBox c)
    {
        c.Items.Clear();
        c.Items.Add("Other Index");
        foreach (var w in a.Banks) c.Items.Add("[" + w.Index + "] - " + w.Name);
    }
    ...
    public static void PopulatePlayerBox(SoundArchive a, ComboBox c)
    {
        c.Items.Clear();
        c.Items.Add("Other Index");
        foreach (var w in a.Players) c.Items.Add("[" + w.Index + "] - " + w.Name);
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().Volume =
                (byte)seqVolumeBox.Value;
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ChannelPriority =
                (byte)seqChannelPriorityBox.Value;
    }
    ...
    {
        if (FileOpen && File != null && !WritingInfo)
            SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().PlayerPriority =
                (byte)seqPlayerPriorityBox.Value;
    }
    ...
    public void SequenceBankComboBoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
            ...
            {
                WritingInfo = true;
                var index = ushort.Parse(((string)seqBankComboBox.SelectedItem).Split('[')[1].Split(']')[0]);
                seqBankBox.Value = index;
                SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ReadingBankId =
                    index;
                SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().Bank =
                    SA.Banks.Where(x => x.Index == index).FirstOrDefault();
                WritingInfo = false;
            }
    }
    ...
    public void SequenceBankBoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            ...
            SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().Bank =
                SA.Banks.Where(x => x.Index == (ushort)seqBankBox.Value).FirstOrDefault();
            SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ReadingBankId =
                (ushort)seqBankBox.Value;
            WritingInfo = false;
        }
    }
    ...
    public void SequencePlayerComboBoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
            ...
            {
                WritingInfo = true;
                var index = byte.Parse(((string)seqPlayerComboBox.SelectedItem).Split('[')[1].Split(']')[0]);
                seqPlayerBox.Value = index;
                SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ReadingPlayerId =
                    index;
                SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().Player =
                    SA.Players.Where(x => x.Index == index).FirstOrDefault();
                WritingInfo = false;
            }
    }
    ...
    public void SequencePlayerBoxChanged(object sender, EventArgs e)
    {
        if (FileOpen && File != null && !WritingInfo)
        {
            ...
            SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().Player =
                SA.Players.Where(x => x.Index == (byte)seqPlayerBox.Value).FirstOrDefault();
            SA.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode)).FirstOrDefault().ReadingPlayerId =
                (byte)seqPlayerBox.Value;
            WritingInfo = false;
        }
    }
    ...
    public void PositionTick(object sender, EventArgs e)
    {
        if (Player != null && PositionBarFree)
            kermalisPosition.Value = Player.GetCurrentPosition() > kermalisPosition.Maximum
    ...
    {
        if (e.Button == MouseButtons.Left) PositionBarFree = false;
    }
    ...
    public void AddAbove(object sender, EventArgs e)
    {
        ...
        {
            var ind = GetNextAvailablePreviousId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxSequenceId,
        ...
        {
            var ind = GetNextAvailablePreviousId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxSequenceArchiveId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
        ...
        {
            var ind = GetNextAvailablePreviousId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxBankId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddBank(ind);
            SA.Banks = SA.Banks.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailablePreviousId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxWaveArchiveId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddWaveArchive(ind);
            SA.WaveArchives = SA.WaveArchives.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailablePreviousId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxPlayerId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
        ...
        {
            var ind = GetNextAvailablePreviousId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxGroupId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddGroup(ind);
            SA.Groups = SA.Groups.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailablePreviousId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxStreamPlayerId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddStreamPlayer(ind);
            SA.StreamPlayers = SA.StreamPlayers.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailablePreviousId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxStreamId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddStream(ind);
            SA.Streams = SA.Streams.OrderBy(x => x.Index).ToList();
    ...
    public void AddBelow(object sender, EventArgs e)
    {
        ...
        if (tree.SelectedNode.Parent.Name.Equals("sequences"))
        {
            var ind = GetNextAvailableForwardId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxSequenceId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddSequence(ind);
            SA.Sequences = SA.Sequences.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailableForwardId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxSequenceArchiveId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddSequenceArchive(ind);
            SA.SequenceArchives = SA.SequenceArchives.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailableForwardId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxBankId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddBank(ind);
            SA.Banks = SA.Banks.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailableForwardId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxWaveArchiveId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddWaveArchive(ind);
            SA.WaveArchives = SA.WaveArchives.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailableForwardId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxPlayerId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddSequencePlayer(ind);
            SA.Players = SA.Players.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailableForwardId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxGroupId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddGroup(ind);
            SA.Groups = SA.Groups.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailableForwardId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxStreamPlayerId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddStreamPlayer(ind);
            SA.StreamPlayers = SA.StreamPlayers.OrderBy(x => x.Index).ToList();
        ...
        {
            var ind = GetNextAvailableForwardId(GetIdFromNode(tree.SelectedNode), SoundArchive.MaxStreamId,
                tree.SelectedNode.Parent.Name);
            if (ind == -1) return;
            AddStream(ind);
            SA.Streams = SA.Streams.OrderBy(x => x.Index).ToList();
    ...
    public void Replace(object sender, EventArgs e)
    {
        ...
        var ind = GetIdFromNode(tree.SelectedNode);
        ...
        if (o.ShowDialog() == DialogResult.OK)
            ...
            switch (Path.GetExtension(o.FileName))
            {
                ...
                //SF2.
                case ".sf2":
                    var sf2 = new SoundFont(o.FileName);
                ...
                //DLS.
                case ".dls":
                    var dls = new DownloadableSounds(o.FileName);
                ...
                //STRM.
                case ".strm":
                    SA.Streams.Where(x => x.Index == ind).FirstOrDefault().File = new Stream();
            ...
            switch (Path.GetExtension(s.FileName))
            {
                ...
                case ".smft":
                    if (tree.SelectedNode.Parent.Parent == null)
                    {
                        ...
                        SA.Sequences.Where(x => x.Index == ind).FirstOrDefault().File.Name =
                            SA.Sequences.Where(x => x.Index == ind).FirstOrDefault().Name;
                        System.IO.File.WriteAllLines(s.FileName,
                            SA.Sequences.Where(x => x.Index == ind).FirstOrDefault().File.ToText());
                    }
                    else
                    {
                        throw new NotImplementedException();
                    ...
                    if (tree.SelectedNode.Parent.Parent == null)
                        switch (seqExportModeBox.SelectedIndex)
                        {
                            ...
                            case 1:
                                ...
                                pro.Start();
                                pro.WaitForExit();
                                if (System.IO.File.Exists(s.FileName) && s.FileName != "temp.mid")
                ...
                case ".wav":
                    ...
                    {
                        var a = SA.SequenceArchives.Where(x => x.Index == GetIdFromNode(tree.SelectedNode.Parent))
                            .FirstOrDefault();
                        var seq = a.File.Sequences.Where(x => x.Index == GetIdFromNode(tree.SelectedNode))
                            .FirstOrDefault();
                        a.File.ReadCommandData(true);
                        try
                        {
                ...
                case ".mus":
                    ...
                    var vals = sa.Labels.Values.ToArray();
                    var bakNames = sa.Labels.Keys.ToArray();
                ...
                //SF2.
                case ".sf2":
                    var sf2 = SA.Banks.Where(x => x.Index == ind).FirstOrDefault().File
                        .ToSoundFont(SA, SA.Banks.Where(x => x.Index == ind).FirstOrDefault());
                    sf2.Write(s.FileName);
                    break;

                //DLS.
                case ".dls":
                    var dls = SA.Banks.Where(x => x.Index == ind).FirstOrDefault().File
                        .ToDLS(SA, SA.Banks.Where(x => x.Index == ind).FirstOrDefault());
    ...
    {
        //Get the new name.
        var newName = Interaction.InputBox("Rename the entry:", "Renamer",
            tree.SelectedNode.Text.Substring(tree.SelectedNode.Text.IndexOf(' ') + 1));
        var index = GetIdFromNode(tree.SelectedNode);
        if (newName == "") return;
        switch (tree.SelectedNode.Parent.Name)
        {
            case "sequences":
                ...
                {
                    MessageBox.Show("An entry of the same name already exists!");
                    return;
                }
        ...
        if (tree.SelectedNode.Parent.Parent != null)
        {
            var sar = SA.SequenceArchives.Where(x => x.Index == GetIdFromNode(tree.SelectedNode.Parent))
                .FirstOrDefault();
    ...
    public void Delete(object sender, EventArgs e)
    {
        switch (tree.SelectedNode.Parent.Name)
        {
            case "sequences":
                ...
                for (var i = 0; i < SA.Groups.Count; i++)
    ...
    public int GetNextAvailableForwardId(int preferredId, uint maxId, string root)
    {
        ...
        //Increment Id.
        while (id <= maxId && rootHasId()) id++;

        //Overflow, start at 0.
        if (id > maxId)
        {
            id = 0;
            while (id < preferredId && rootHasId()) id++;
            if (id == preferredId)
            {
    ...
    public override void RootAdd()
    {
        ...
        {
            var ind = GetNextAvailableForwardId(SA.Sequences.Count > 0 ? SA.Sequences.Last().Index + 1 : 0,
                SoundArchive.MaxSequenceId, tree.SelectedNode.Name);
            if (ind == -1) return;
            AddSequence(ind);
            SA.Sequences = SA.Sequences.OrderBy(x => x.Index).ToList();
            UpdateNodes();
        ...
        {
            var ind = GetNextAvailableForwardId(
                SA.SequenceArchives.Count > 0 ? SA.SequenceArchives.Last().Index + 1 : 0,
                SoundArchive.MaxSequenceArchiveId, tree.SelectedNode.Name);
            if (ind == -1) return;
            AddSequenceArchive(ind);
            SA.SequenceArchives = SA.SequenceArchives.OrderBy(x => x.Index).ToList();
            UpdateNodes();
        ...
        {
            var ind = GetNextAvailableForwardId(SA.Banks.Count > 0 ? SA.Banks.Last().Index + 1 : 0,
                SoundArchive.MaxBankId, tree.SelectedNode.Name);
            if (ind == -1) return;
            AddBank(ind);
            SA.Banks = SA.Banks.OrderBy(x => x.Index).ToList();
            UpdateNodes();
        ...
        {
            var ind = GetNextAvailableForwardId(SA.WaveArchives.Count > 0 ? SA.WaveArchives.Last().Index + 1 : 0,
                SoundArchive.MaxWaveArchiveId, tree.SelectedNode.Name);
            if (ind == -1) return;
            AddWaveArchive(ind);
            SA.WaveArchives = SA.WaveArchives.OrderBy(x => x.Index).ToList();
            UpdateNodes();
        ...
        {
            var ind = GetNextAvailableForwardId(SA.Players.Count > 0 ? SA.Players.Last().Index + 1 : 0,
                SoundArchive.MaxPlayerId, tree.SelectedNode.Name);
            if (ind == -1) return;
            AddSequencePlayer(ind);
            SA.Players = SA.Players.OrderBy(x => x.Index).ToList();
            UpdateNodes();
        ...
        {
            var ind = GetNextAvailableForwardId(SA.Groups.Count > 0 ? SA.Groups.Last().Index + 1 : 0,
                SoundArchive.MaxGroupId, tree.SelectedNode.Name);
            if (ind == -1) return;
            AddGroup(ind);
            SA.Groups = SA.Groups.OrderBy(x => x.Index).ToList();
            UpdateNodes();
        ...
        {
            var ind = GetNextAvailableForwardId(SA.StreamPlayers.Count > 0 ? SA.StreamPlayers.Last().Index + 1 : 0,
                SoundArchive.MaxStreamPlayerId, tree.SelectedNode.Name);
            if (ind == -1) return;
            AddStreamPlayer(ind);
            SA.StreamPlayers = SA.StreamPlayers.OrderBy(x => x.Index).ToList();
            UpdateNodes();
        ...
        {
            var ind = GetNextAvailableForwardId(SA.Streams.Count > 0 ? SA.Streams.Last().Index + 1 : 0,
                SoundArchive.MaxStreamId, tree.SelectedNode.Name);
            if (ind == -1) return;
            AddStream(ind);
            SA.Streams = SA.Streams.OrderBy(x => x.Index).ToList();
            UpdateNodes();
    ...
    public void AddSequence(int index)
    {
        ...
        //Add the sequence.
        var e = new SequenceInfo();
        e.Bank = SA.Banks[0];
        e.Player = SA.Players[0];
        e.Name = "SEQ_" + index;
        e.Index = index;
        var nameIndex = index;
        while (SA.Sequences.Where(x => x.Name.Equals("SEQ_" + nameIndex)).Count() > 0) e.Name = "SEQ_" + nameIndex++;
    ...
    public void AddBank(int index)
    {
        ...
        e.Index = index;
        var nameIndex = index;
        while (SA.Banks.Where(x => x.Name.Equals("BANK_" + nameIndex)).Count() > 0) e.Name = "BANK_" + nameIndex++;
    ...
    public void AddWaveArchive(int index)
    {
        ...
        var e = new WaveArchiveInfo();
        e.File = new WaveArchive();
        e.Name = "WAR_" + index;
        e.Index = index;
        var nameIndex = index;
        while (SA.WaveArchives.Where(x => x.Name.Equals("WAR_" + nameIndex)).Count() > 0) e.Name = "WAR_" + nameIndex++;
    ...
    public void AddGroup(int index)
    {
        ...
        var nameIndex = index;
        while (SA.Groups.Where(x => x.Name.Equals("GROUP_" + nameIndex)).Count() > 0) e.Name = "GROUP_" + nameIndex++;
    ...
    public void AddStream(int index)
    {
        ...
        e.File = s;
        var nameIndex = index;
        while (SA.Streams.Where(x => x.Name.Equals("STRM_" + nameIndex)).Count() > 0) e.Name = "STRM_" + nameIndex++;
    ...
    public void ReplaceBankWithDLS(BankInfo b, DownloadableSounds d)
    {
        ...
        var wavSamples = new List<RiffWave>();
        ...
        var wars = b.WaveArchives.Where(x => x != null).ToList();
        var otherWavId = new Dictionary<uint, int>();
        ...
        //Add waves.
        var swavMap = new Dictionary<int, Tuple<ushort, ushort>>();
        foreach (var w in wavSamples)
        {
            ...
            var md5 = wav.Md5Sum;
            if (war.File.Waves.Where(x => x.Md5Sum.Equals(md5)).Count() < 1) war.File.Waves.Add(wav);
            swavMap.Add(wavSamples.IndexOf(w),
                new Tuple<ushort, ushort>((ushort)b.WaveArchives.ToList().IndexOf(war),
                    (ushort)war.File.Waves.IndexOf(war.File.Waves.Where(x => x.Md5Sum.Equals(md5)).FirstOrDefault())));
        }
                ...
                foreach (var c in a.Connections)
                {
                    if (c.DestinationConnection == DestinationConnection.EG1AttackTime)
                        if (c.Scale != int.MinValue)
                            n.Attack = Bank.GetNearestTableIndex(Bank.TimecentsToMilliseconds(c.Scale / 65536),
                                Bank.AttackTable);
                    if (c.DestinationConnection == DestinationConnection.EG1DecayTime)
                        if (c.Scale != int.MinValue)
                            n.Decay = Bank.GetNearestTableIndex(Bank.TimecentsToMilliseconds(c.Scale / 65536),
                    ...
                    if (c.DestinationConnection == DestinationConnection.EG1ReleaseTime)
                        if (c.Scale != int.MinValue)
                            n.Release = Bank.GetNearestTableIndex(Bank.TimecentsToMilliseconds(c.Scale / 65536),
                                Bank.MaxReleaseTimes);
                    if (c.DestinationConnection == DestinationConnection.Pan) n.Pan = Bank.SetPan(c.Scale / 65536);
                }
    ...
    public override void exportFileToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        if (s.ShowDialog() == DialogResult.OK) SA.Write(s.FileName);
    }
}