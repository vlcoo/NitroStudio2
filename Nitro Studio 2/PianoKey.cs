...
public class PianoKey : Control
{
    ...
    private void InitPointsHorz()
    {
        ...
        if (Orientation == PianoKeyOrientation.HorizontalRight) FlipHorizontally();
    }
    ...
    {
        for (var i = 0; i < points.Length; i++) points[i].X = Size.Width - points[i].X;
    }
    ...
    {
        for (var i = 0; i < points.Length; i++) points[i].Y = Size.Height - points[i].Y;
    }
    ...
    private void CreateRegion()
    {
        ...
        for (var i = 0; i < types.Length; i++) types[i] = (byte)PathPointType.Line;

        var path = new GraphicsPath(points, types);
